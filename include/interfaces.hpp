...
{
public:
    inline static void init(const so::shared_object& object, const std::string& name, int version)
    {
        ...
        for (int i = version; i < 100; i++) {
            ...
            interface_pointer_ = reinterpret_cast<T*>(object.create_interface_fn(ifname, nullptr));
